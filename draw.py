...
def scanline_convert(polygons, i, screen, zbuffer, color):
    ...
    dx0 = (
        points[TOP][0] - points[BOT][0]) / distance0 if distance0 != 0 else 0
    dz0 = (
        points[TOP][2] - points[BOT][2]) / distance0 if distance0 != 0 else 0
    dx1 = (
        points[MID][0] - points[BOT][0]) / distance1 if distance1 != 0 else 0
    dz1 = (
        points[MID][2] - points[BOT][2]) / distance1 if distance1 != 0 else 0
...
def draw_polygons(matrix, screen, zbuffer, view, ambient, light, areflect,
                  dreflect, sreflect):
    if len(matrix) < 2:
    ...
    while point < len(matrix) - 2:
        ...
        if dot_product(normal, view) > 0:
            ...
            color = get_lighting(normal, view, ambient, light, areflect,
                                 dreflect, sreflect)
...
def draw_lines(matrix, screen, zbuffer, color):
    ...
    while point < len(matrix) - 1:
        draw_line(
            int(matrix[point][0]), int(matrix[point][1]), matrix[point][2],
            int(matrix[point + 1][0]), int(matrix[point + 1][1]),
            matrix[point + 1][2], screen, zbuffer, color)
        point += 2
...
def draw_line(x0, y0, z0, x1, y1, z1, screen, zbuffer, color):
    ...
    while (loop_start < loop_end):
        ...
        if ((wide and ((A > 0 and d > 0) or (A < 0 and d < 0)))
                or (tall and ((A > 0 and d < 0) or (A < 0 and d > 0)))):
